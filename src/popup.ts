...
function updateTranslations(): void {
  ...
  const providerLabel = document.querySelector(
    'label[for="provider-selector"]',
  );
...
function initializeProviderManagement(): void {
  const providerSelector = document.getElementById(
    "provider-selector",
...
function updateProviderStatus(providerId: string): void {
  ...
  const statusElement = document.querySelector(
    `#${providerId}-config .provider-status-indicator`,
  );
  if (statusElement) {
...
function showManualModelForm(): void {
  ...
  if (manualModelSection) {
    ...
    // Clear form
    const modelIdInput = document.getElementById(
      "manual-model-id",
    ) as HTMLInputElement;
    const modelNameInput = document.getElementById(
      "manual-model-name",
    ) as HTMLInputElement;
...
function initializeSettings(): void {
  const showIconCheckbox = document.getElementById(
    "show-icon",
  ) as HTMLInputElement;
...
function initializeCustomPrompts(): void {
  const addPromptButton = document.querySelector(
    ".ai-add-prompt",
  ) as HTMLButtonElement;
...
function addPromptItem(command: string, text: string): void {
  ...
  const saveButton = promptItem.querySelector(
    ".save-prompt",
  ) as HTMLButtonElement;
  const deleteButton = promptItem.querySelector(
    ".delete-prompt",
  ) as HTMLButtonElement;
  const commandInput = promptItem.querySelector(
    ".prompt-command",
  ) as HTMLInputElement;
  const textInput = promptItem.querySelector(
    ".prompt-text",
