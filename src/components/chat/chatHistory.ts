...
export async function loadChatHistory(): Promise<void> {
  ...
  try {
    ...
    currentHistory = urlHistory
      ? { ...urlHistory }
      : { messages: [], url: currentUrl };
...
export async function updateLastMessage(content: string): Promise<void> {
  ...
  const lastMessage =
    currentHistory.messages[currentHistory.messages.length - 1];
  ...
  try {
    ...
    const answerDiv = document.getElementById("ai-answer");
    if (answerDiv) {
      const lastMessageDiv = answerDiv.querySelector(
        ".ai-chat-messages .ai-assistant-message:last-of-type .ai-message-content",
      ) as HTMLElement;
      if (lastMessageDiv) {
...
export function getChatHistoryMessages(): Array<{
...
function createMessageElement(message: ChatMessage): HTMLDivElement {
  ...
  if (message.role === "assistant") {
    ...
    if (
      message === currentHistory.messages[currentHistory.messages.length - 1]
    ) {
      ...
      const model = modelManager
        .getAllModels()
        .find((m) => m.id === message.model);
      const displayName = model
        ? model.name
        : MODEL_DISPLAY_NAMES[message.model as ModelDisplayType] ||
          message.model ||
          "Unknown Model";
