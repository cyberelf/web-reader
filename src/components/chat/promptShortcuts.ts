...
let PROMPT_SHORTCUTS: PromptShortcuts = {
  "/summarize":
    "Please provide a concise summary of this content, highlighting the main points and key takeaways.",
  "/explain":
    "Please explain this content in simple terms, breaking down any complex concepts and providing clear explanations.",
  "/generate":
    "Please analyze the style and tone of this content, then generate a new piece of text that matches this style but covers a similar topic.",
};

async function loadCustomPrompts() {
  try {
    if (typeof chrome !== "undefined" && chrome.storage?.sync) {
      const { customPrompts = {} } = await chrome.storage.sync.get([
        "customPrompts",
      ]);
      PROMPT_SHORTCUTS = {
        ...PROMPT_SHORTCUTS,
