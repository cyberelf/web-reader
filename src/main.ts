...
import {
  setupContextModes,
  clearVideoSubtitles,
} from "./components/context/contextModes";
  ...
  try {
    ...
    function trySetupContextModes() {
      ...
      if (sidebar && contentPreview) {
        ...
        setupAttempts++;
        console.log(
          `Waiting for sidebar elements, attempt ${setupAttempts}/${maxSetupAttempts}`,
        );
        setTimeout(trySetupContextModes, 200);
      } else {
        console.error(
          "Failed to initialize extension: Required elements not found after",
          maxSetupAttempts,
          "attempts",
        );
      }
  ...
  function openChartInNewTab(diagramId: string) {
    ...
    try {
      ...
      const chartTypeLabel =
        container.querySelector(".chart-type-label")?.textContent || "Chart";
