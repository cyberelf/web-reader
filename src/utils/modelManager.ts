...
const DEFAULT_MODELS: Record<string, ModelInfo[]> = {
  openai: [
    { id: 'gpt-4o', name: 'GPT-4O', provider: 'openai', isManual: false, isVision: true },
    { id: 'gpt-4o-mini', name: 'GPT-4O Mini', provider: 'openai', isManual: false, isVision: true },
    { id: 'gpt-4-turbo', name: 'GPT-4 Turbo', provider: 'openai', isManual: false, isVision: true },
    { id: 'gpt-3.5-turbo', name: 'GPT-3.5 Turbo', provider: 'openai', isManual: false }
  ],
  deepseek: [
    { id: 'deepseek-chat', name: 'DeepSeek V3', provider: 'deepseek', isManual: false },
    { id: 'deepseek-reasoner', name: 'DeepSeek R1', provider: 'deepseek', isManual: false }
  ],
  gemini: [
    { id: 'gemini-2.5-pro-preview-05-06', name: 'Gemini 2.5 Pro Preview', provider: 'gemini', isManual: false, isVision: true },
    { id: 'gemini-2.5-flash-preview-05-20', name: 'Gemini 2.5 Flash Preview', provider: 'gemini', isManual: false, isVision: true },
    { id: 'gemini-2.0-flash-exp', name: 'Gemini 2.0 Flash (Experimental)', provider: 'gemini', isManual: false, isVision: true },
    { id: 'gemini-2.0-flash', name: 'Gemini 2.0 Flash', provider: 'gemini', isManual: false, isVision: true },
    { id: 'gemini-1.5-pro', name: 'Gemini 1.5 Pro', provider: 'gemini', isManual: false, isVision: true },
    { id: 'gemini-1.5-flash', name: 'Gemini 1.5 Flash', provider: 'gemini', isManual: false, isVision: true },
    { id: 'gemini-1.5-flash-8b', name: 'Gemini 1.5 Flash 8B', provider: 'gemini', isManual: false, isVision: true }
  ]
};
...
export class ModelManager {
  ...
  private fixExistingModels(): void {
    ...
    Object.keys(this.data.models).forEach(providerId => {
      this.data.models[providerId] = this.data.models[providerId].map(model => {
        ...
        if (model.isManual === undefined) {
          ...
          const isDefaultModel = defaultModels.some(defaultModel => defaultModel.id === model.id);
          return {
            ...model,
  ...
  private async saveData(): Promise<void> {
    try {
      ...
      Object.keys(this.data.models).forEach(providerId => {
        optimizedModels[providerId] = this.data.models[providerId].map(model => ({
          id: model.id,
          name: model.name,
      ...
      await Promise.all([
        new Promise<void>((resolve, reject) => {
          chrome.storage.sync.set({ [this.storageKey]: mainData }, () => {
            if (chrome.runtime.lastError) {
              console.error('Failed to save main data:', chrome.runtime.lastError);
        ...
        new Promise<void>((resolve, reject) => {
          chrome.storage.sync.set({ [this.modelsStorageKey]: optimizedModels }, () => {
            if (chrome.runtime.lastError) {
              console.error('Failed to save models data:', chrome.runtime.lastError);
  ...
  private async saveEssentialModelsOnly(): Promise<void> {
    ...
    Object.keys(this.data.models).forEach(providerId => {
      ...
      essentialModels[providerId] = [...manualModels, ...defaultModels].map(model => ({
        id: model.id,
        name: model.name,
        provider: model.provider,
        isManual: model.isManual,
    ...
    return new Promise((resolve, reject) => {
      chrome.storage.sync.set({ [this.modelsStorageKey]: essentialModels }, () => {
        if (chrome.runtime.lastError) {
          console.error('Failed to save essential models:', chrome.runtime.lastError);
          reject(chrome.runtime.lastError);
        } else {
          resolve();
        }
  ...
  addManualModel(providerId: string, model: Omit<ModelInfo, 'provider' | 'isManual'>): void {
    ...
    const existingIndex = this.data.models[providerId].findIndex(m => m.id === model.id);
    if (existingIndex >= 0) {
  ...
  removeModel(providerId: string, modelId: string): void {
    if (this.data.models[providerId]) {
      const modelIndex = this.data.models[providerId].findIndex(m => m.id === modelId);
      ...
      if (modelIndex >= 0) {
        ...
        if (model.isManual) {
          ...
          throw new Error('Cannot delete auto-synced models. Only manually added models can be deleted.');
        }
      } else {
  ...
  // Model syncing
  async syncModels(providerId: string): Promise<{ success: boolean; error?: string; models?: ModelInfo[] }> {
    ...
    try {
      ...
      const manualModels = this.data.models[providerId]?.filter(m => m.isManual) || [];
  ...
  private async fetchModelsFromAPI(provider: ProviderConfig): Promise<ModelInfo[]> {
  ...
  private async fetchOpenAIModels(provider: ProviderConfig): Promise<ModelInfo[]> {
    try {
  ...
  private async fetchGeminiModels(provider: ProviderConfig): Promise<ModelInfo[]> {
    try {
      ...
      return models.length > 0 ? models : DEFAULT_MODELS.gemini.map(m => ({ ...m, isManual: false }));
    } catch (error) {
  ...
  private async fetchOpenAICompatibleModels(provider: ProviderConfig): Promise<ModelInfo[]> {
    try {
      const response = await fetch(`${provider.apiUrl}/models`, {
        headers: {
          'Authorization': `Bearer ${provider.apiKey}`,
  ...
  private formatModelName(modelId: string): string {
    ...
    return nameMap[modelId] || modelId.split('-').map(word => 
  ...
  getCurrentConfig(): { apiKey?: string; apiUrl: string; model?: string } | null {
  ...
  async cleanupOldStorage(): Promise<void> {
    return new Promise((resolve) => {
      chrome.storage.sync.remove(['settings', 'customModels', this.storageKey, this.modelsStorageKey], resolve);
    });
  }
}
