...
Root
  ...
  describe('Unsupported Diagram Types', () => {
    ...
    John-->>Alice: Hi Alice`;
      ...
      expect(result).toContain('Chart rendering failed: Diagram type not supported by lightweight renderer');
    });
  ...
  describe('SVG Rendering and Positioning', () => {
    describe('Flowchart Layout Validation', () => {
      it('should render nodes at different vertical levels in top-down flowchart', () => {
        ...
        const yPositions = Array.from(rectMatches).map(match => parseFloat(match[1]));
        expect(yPositions.length).toBeGreaterThan(1);
        
        // Verify nodes are at different levels (sorted by dependency)
        const uniqueYPositions = [...new Set(yPositions.map(y => Math.round(y / 50) * 50))];
      ...
      it('should render horizontal flowchart with proper X positioning', () => {
        ...
        const xPositions = Array.from(rectMatches).map(match => parseFloat(match[1]));
      ...
      it('should render circular nodes correctly', () => {
        ...
        const circleMatches = result.matchAll(/<circle[^>]*cx="([^"]+)"[^>]*cy="([^"]+)"/g);
      ...
      it('should handle vertical spacing with long node labels', () => {
        ...
        const rectMatches = result.matchAll(/<rect[^>]*x="([^"]+)"[^>]*y="([^"]+)"[^>]*width="([^"]+)"[^>]*height="([^"]+)"/g);
        ...
        const polygonMatches = result.matchAll(/<polygon[^>]*points="([^"]+)"/g);
        const polygonNodes = Array.from(polygonMatches).map(match => {
          const points = match[1].split(' ').map(p => p.split(',').map(parseFloat));
      ...
      it('should handle complex branching without node crowding', () => {
        ...
        for (let i = 0; i < nodes.length; i++) {
          for (let j = i + 1; j < nodes.length; j++) {
            ...
            const xOverlap = Math.max(0, Math.min(node1.right, node2.right) - Math.max(node1.x, node2.x));
            const yOverlap = Math.max(0, Math.min(node1.bottom, node2.bottom) - Math.max(node1.y, node2.y));
            
            // Consider significant if overlap is more than 25% of smaller node
            const minArea = Math.min(node1.width * node1.height, node2.width * node2.height);
    ...
    describe('Mindmap Multi-level Tree Validation', () => {
      it('should render multi-level mindmap with proper hierarchical positioning', () => {
        ...
        const rectMatches = result.matchAll(/<rect[^>]*x="([^"]+)"[^>]*y="([^"]+)"/g);
        ...
        const uniqueXPositions = [...new Set(xPositions.map(x => Math.round(x / 100) * 100))]; // Group by ~100px intervals
      ...
      it('should position mindmap nodes to avoid overlap', () => {
        ...
        for (let i = 0; i < nodes.length; i++) {
          for (let j = i + 1; j < nodes.length; j++) {
            ...
            const minNodeSize = Math.min(node1.width, node1.height, node2.width, node2.height);
      ...
      it('should render correct parent-child connections in deep hierarchy', () => {
        ...
        const pathMatches = result.matchAll(/<path[^>]*d="M\s*([\d.-]+)\s*([\d.-]+)\s*C[^"]*?([\d.-]+)\s*([\d.-]+)"/g);
        ...
        connections.forEach(conn => {
          const fromNode = nodes.find(n => Math.abs(n.centerX - conn.fromX) < 80 && Math.abs(n.centerY - conn.fromY) < 30);
          const toNode = nodes.find(n => Math.abs(n.centerX - conn.toX) < 80 && Math.abs(n.centerY - conn.toY) < 30);
          
          if (fromNode && toNode && Math.abs(fromNode.level - toNode.level) === 1) {
      ...
      it('should position sibling nodes vertically separated at same level', () => {
        ...
        const nodesByLevel: Record<number, Array<{x: number, y: number}>> = {};
      ...
      it('should verify grandchildren connect to parents not grandparents', () => {
        ...
        const uniqueXPositions = [...new Set(xPositions.map(x => Math.round(x / 100) * 100))];
        ...
        connections.forEach(conn => {
          const fromNode = nodes.find(n => 
            Math.abs(n.centerX - conn.fromX) < 80 && Math.abs(n.centerY - conn.fromY) < 40
          );
          const toNode = nodes.find(n => 
            Math.abs(n.centerX - conn.toX) < 80 && Math.abs(n.centerY - conn.toY) < 40
          );
          
          if (fromNode && toNode) {
            if (fromNode.level === 1 && toNode.level === 2) {  // Parent to child
              childToParentConnections++;
            } else if (fromNode.level === 0 && toNode.level === 2) {  // Grandparent to grandchild (should be 0)
      ...
      it('should render consistent level spacing across branches', () => {
        ...
        const uniqueXPositions = [...new Set(xPositions.map(x => Math.round(x / 100) * 100))]; // Group by ~100px
        ...
        nodes.forEach(node => {
          const levelIndex = uniqueXPositions.findIndex(x => Math.abs(node.x - x) < 50);
      ...
      describe('Vertical Overlap Prevention', () => {
        ...
        it('should handle deep hierarchy without vertical crowding', () => {
          ...
          // Parse node positions
          const rectMatches = result.matchAll(/<rect[^>]*x="([^"]+)"[^>]*y="([^"]+)"[^>]*height="([^"]+)"/g);
          const nodes = Array.from(rectMatches).map(match => ({
            x: parseFloat(match[1]),
            y: parseFloat(match[2]),
          ...
          const nodesByLevel: Record<number, Array<{x: number, y: number, height: number, bottom: number}>> = {};
          nodes.forEach(node => {
        ...
        it('should handle many sibling nodes at same level without overlap', () => {
          ...
          const totalHeight = sortedSiblings[sortedSiblings.length - 1].bottom - sortedSiblings[0].y;
    ...
    describe('Pie Chart Arc Positioning', () => {
      ...
      it('should render pie chart legend with proper positioning', () => {
        ...
        const legendRectMatches = result.matchAll(/<rect[^>]*x="320"[^>]*y="([^"]+)"/g);
        const legendPositions = Array.from(legendRectMatches).map(match => parseFloat(match[1]));
        ...
        for (let i = 1; i < legendPositions.length; i++) {
          expect(legendPositions[i] - legendPositions[i-1]).toBeGreaterThan(30); // Minimum spacing
        }
      });
    });
