 ...
 */
export function simpleConversationTokens(
  messages: any[],
  model = "gpt-4o-mini",
): SimpleTokenEstimate {
 ...
 */
export function simpleTokenEstimate(
  question: string,
  context: string,
  model = "gpt-4o-mini",
): SimpleTokenEstimate {
...
export function formatSimpleTokenCount(estimate: SimpleTokenEstimate): string {
  ...
  if (estimate.isEstimated) {
    return (
      parts.join(" + ") +
      ` : ${estimate.totalTokens.toLocaleString()} total tokens` +
      " (est)"
    );
  }
 ...
 */
export function getSimpleTokenStatus(estimate: SimpleTokenEstimate): {
  text: string;
