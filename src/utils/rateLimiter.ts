...
export class RateLimiter {
  ...
  async canMakeRequest(estimatedTokens: number = 1000): Promise<{ allowed: boolean; reason?: string; waitTime?: number }> {
    ...
    if (this.config.tokensPerMinute && stats.tokensLastMinute + estimatedTokens > this.config.tokensPerMinute) {
      const waitTime = 60000 - (now - stats.lastRequestTime);
      return {
        allowed: false,
    ...
    if (this.config.tokensPerDay && stats.tokensLastDay + estimatedTokens > this.config.tokensPerDay) {
      return {
        allowed: false,
  ...
  async recordRequest(actualTokens: number): Promise<void> {
    ...
    data.requests = data.requests.filter(record => record.timestamp > oneDayAgo);
  ...
  async getUsageStats(): Promise<UsageStats> {
    ...
    const recentRequests = data.requests.filter(record => record.timestamp > oneDayAgo);
    
    const requestsLastMinute = recentRequests.filter(record => record.timestamp > oneMinuteAgo).length;
    const requestsLastHour = recentRequests.filter(record => record.timestamp > oneHourAgo).length;
    ...
    const tokensLastDay = recentRequests
      .reduce((sum, record) => sum + record.tokens, 0);
  ...
  private async saveStorageData(data: { requests: RequestRecord[] }): Promise<void> {
    return new Promise((resolve) => {
