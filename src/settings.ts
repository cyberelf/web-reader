...
function updateTokenDisplay(usage: TokenUsage): void {
  ...
  if (totalTokensEl)
    totalTokensEl.textContent = usage.totalTokens.toLocaleString();
  if (requestCountEl)
    requestCountEl.textContent = usage.requestCount.toLocaleString();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  getSettings().then((settings) => {
    ...
    if (settings.apiUrl) {
      (document.getElementById("openai-url") as HTMLInputElement).value =
        settings.apiUrl;
    }

    if (settings.showIcon !== undefined) {
      (document.getElementById("show-icon") as HTMLInputElement).checked =
        settings.showIcon;
    }
  });
  ...
  document
    ...
    ?.addEventListener("click", async () => {
      const settings: Partial<Settings> = {
        ...
        showIcon: (document.getElementById("show-icon") as HTMLInputElement)
          .checked,
      };
